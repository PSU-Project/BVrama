...
Image ReadPGM(FILE *fp)
{
  ...
  char2 = fgetc(fp);
  SkipComments(fp);
  c1 = fscanf_s(fp, "%d", &width);
  SkipComments(fp);
  c2 = fscanf_s(fp, "%d", &height);
  SkipComments(fp);
...
Keypoint ReadKeys(FILE *fp)
{
    ...
    for (i = 0; i < num; i++) {
      ...
      if (fscanf_s(fp, "%f %f %f %f", &(k->row), &(k->col), &(k->scale),
		 &(k->ori)) != 4)
	FatalError("Invalid keypoint file format.");

      for (j = 0; j < len; j++) {
	if (fscanf_s(fp, "%d", &val) != 1 || val < 0 || val > 255)
